...
_INLINE_ void generate_map_two(OUT map_word_t *map, IN const map_word_t l_param)
{
  __m256i vmap[NUM_YMMS], vtmp[NUM_YMMS], vr, inc, zero;
  ...
  //   4.   if map[i] < 0:
  //   5.     map[i] = map[i] + r
  inc = SET1((l_param * NUM_OF_VALS) % R_BITS_TWO);
  inc = SUB(inc, vr);
...
_INLINE_ void bytes_to_bin_two(OUT pad_r_t_two *bin_buf, IN const uint8_t *bytes_buf)
{
  ...
  for(size_t i = 0; i < R_QWORDS_TWO * 2; i++) {
    __m256i t = LOAD(&bytes_buf[i * BYTES_IN_YMM]);
    bin32[i]  = MOVEMASK(t);
  }
}
...
void k_sqr_avx2_two(OUT pad_r_t_two *c, IN const pad_r_t_two *a, IN const size_t l_param)
{
  ...
  // Generate the permutation map defined by pi1 and l_param.
  generate_map_two(map, l_param);

  bin_to_bytes_two(a_bytes, a);

  // Permute "a" using the generated permutation map.
  for(size_t i = 0; i < R_BITS_TWO; i++) {
    c_bytes[i] = a_bytes[map[i]];
  }
