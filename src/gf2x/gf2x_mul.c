...
#define SECURE_BUFFER_QWORDS_TWO (3 * R_PADDED_QWORDS_TWO)
...
_INLINE_ void karatzuba_two(OUT uint64_t *c,
                        IN const uint64_t *a,
                        IN const uint64_t *b,
                        IN const size_t    qwords_len,
                        IN const size_t    qwords_len_pad,
                        ...
                        IN const gf2x_ctx_two *ctx)
{
  ...
  // Compute a_lo*b_lo and store the result in (c1|c0)
  karatzuba_two(c0, a_lo, b_lo, half_qw_len, half_qw_len, sec_buf, ctx);
  ...
  if(qwords_len > half_qw_len) {
    // Compute a_hi*b_hi and store the result in (c3|c2)
    karatzuba_two(c2, a_hi, b_hi, qwords_len - half_qw_len, half_qw_len, sec_buf,
              ctx);
    ...
    // Compute alah*blbh and store the result in (c2|c1)
    karatzuba_two(c1, alah, blbh, half_qw_len, half_qw_len, sec_buf, ctx);
...
void gf2x_mod_mul_with_ctx_two(OUT pad_r_t_two *c,
                           ...
                           IN const gf2x_ctx_two *ctx)
{
  bike_static_assert((R_PADDED_BYTES_TWO % 2 == 0), karatzuba_n_is_odd);

  DEFER_CLEANUP(dbl_pad_r_t_two t = {0}, dbl_pad_r_two_cleanup);
  ALIGN(ALIGN_BYTES) uint64_t secure_buffer[SECURE_BUFFER_QWORDS_TWO];

  karatzuba_two((uint64_t *)&t, (const uint64_t *)a, (const uint64_t *)b, R_QWORDS_TWO,
            R_PADDED_QWORDS_TWO, secure_buffer, ctx);
...
void gf2x_mod_mul_two(OUT pad_r_t_two *c, IN const pad_r_t_two *a, IN const pad_r_t_two *b)
{
  ...
  gf2x_mod_mul_with_ctx_two(c, a, b, &ctx);
}