...
typedef struct sampling_ctx_st {
  ...
  void (*secure_set_bits_two)(OUT pad_r_t_two *r,
                          IN size_t    first_pos,
                          IN const idx_t *wlist,
                          IN size_t       w_size);
...
_INLINE_ void sampling_ctx_init(sampling_ctx *ctx)
{
  ...
  if(is_avx512_enabled()) {
    ...
    ctx->secure_set_bits_two = secure_set_bits_avx512_two;
#if defined(UNIFORM_SAMPLING)
    ...
    ctx->secure_set_bits_two = secure_set_bits_avx2_two;
#if defined(UNIFORM_SAMPLING)
...
#endif
  {
    ...
    ctx->secure_set_bits_two = secure_set_bits_port_two;
#if defined(UNIFORM_SAMPLING)
